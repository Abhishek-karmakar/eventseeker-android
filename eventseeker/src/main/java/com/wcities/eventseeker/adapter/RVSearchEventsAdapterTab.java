...
public class RVSearchEventsAdapterTab extends RVAdapterBase<RVSearchEventsAdapterTab.ViewHolder> implements DateWiseEventParentAdapterListener {
	...
	private SearchEventsFragmentTab searchEventsFragmentTab;
	...
	public RVSearchEventsAdapterTab(SearchEventsFragmentTab searchEventsFragmentTab) {
		this.searchEventsFragmentTab = searchEventsFragmentTab;
		
		eventList = searchEventsFragmentTab.getEventList();
	...
	public void onBindViewHolder(final ViewHolder holder, final int position) {
		...
		if (event == null) {
			...
			if (eventList.size() == 1) {
				...
				((FullScrnProgressListener) searchEventsFragmentTab).displayFullScrnProgress();
			...
			if ((loadDateWiseEvents == null || loadDateWiseEvents.getStatus() == Status.FINISHED) && 
				...
				searchEventsFragmentTab.loadItemsInBackground();
			}
			
		} else {
			if (event.getId() == AppConstants.INVALID_ID) {
				searchEventsFragmentTab.displayNoItemsFound();
				...
				final Resources res = FragmentUtil.getResources(searchEventsFragmentTab);
	...
	private void onHandleClick(final ViewHolder holder, final int position) {
		...
		if (holder.isSliderClose()) {
			...
			Animation slide = AnimationUtils.loadAnimation(FragmentUtil.getActivity(
					searchEventsFragmentTab), R.anim.slide_in_from_left);
			...
			Animation slide = AnimationUtils.loadAnimation(FragmentUtil.getActivity(
					searchEventsFragmentTab), android.R.anim.slide_out_right);
			slide.setAnimationListener(new AnimationListener() {
	...
	private void onImgSaveClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				EventSeekr eventSeekr = (EventSeekr) FragmentUtil.getActivity(searchEventsFragmentTab).getApplication();
				if (event.getAttending() == Attending.SAVED) {
	    			...
	    			updateImgSaveSrc(holder, event, FragmentUtil.getResources(searchEventsFragmentTab));
					
				} else {
					searchEventsFragmentTab.setEvent(event);
	...
	private void onImgShareClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				 ...
				 */
				shareViaDialogFragment.show(((BaseActivityTab)FragmentUtil.getActivity(searchEventsFragmentTab))
	...
	private void onEventClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				((EventListenerTab) FragmentUtil.getActivity(searchEventsFragmentTab)).onEventSelected(event, 
	...
	public void onPublishPermissionGranted() {
		...
		updateImgSaveSrc(holderPendingPublish, eventPendingPublish, FragmentUtil.getResources(searchEventsFragmentTab));
	}
