...
import com.wcities.eventseeker.adapter.RVMyEventsAdapterTab.ViewHolder;
...
public class RVMyEventsAdapterTab extends Adapter<ViewHolder> implements DateWiseEventParentAdapterListener {

	private static final String TAG = RVMyEventsAdapterTab.class.getSimpleName();

	private static final int INVALID = -1;
	
	private PublishEventFragment publishEventFragment;
	...
	private RVMyEventsAdapterTab.ViewHolder holderPendingPublish;
	...
	public interface RVMyEventsAdapterTabListener {
	...
	public RVMyEventsAdapterTab(List<Event> eventList, AsyncTask<Void, Void, List<Event>> loadDateWiseEvents,
			LoadItemsInBackgroundListener mListener, PublishEventFragment publishEventFragment, 
			RVMyEventsAdapterTabListener saveEventInstanceListener) {
		
		this.eventList = eventList;
		this.loadDateWiseEvents = loadDateWiseEvents;
		this.mListener = mListener;
		this.publishEventFragment = publishEventFragment;
		this.rvMyEventsAdapterTabListener = saveEventInstanceListener;
	...
	public void onBindViewHolder(final ViewHolder holder, final int position) {
		...
		if (event == null) {
			...
			final Resources res = FragmentUtil.getResources(publishEventFragment);
	...
	private void onImgTicketClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				EventSeekr eventSeekr = FragmentUtil.getApplication(publishEventFragment);
	...
	private void onImgSaveClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				EventSeekr eventSeekr = (EventSeekr) FragmentUtil.getActivity(publishEventFragment).getApplication();
				if (event.getAttending() == Attending.SAVED) {
	    			...
	    			updateImgSaveSrc(holder, event, FragmentUtil.getResources(publishEventFragment));
					
				} else {
					rvMyEventsAdapterTabListener.setEvent(event);
					eventPendingPublish = event;
					holderPendingPublish = holder;
					
					if (eventSeekr.getGPlusUserId() != null) {
						event.setNewAttending(Attending.SAVED);
						publishEventFragment.handlePublishEvent();
						
					} else {
						fbCallCountForSameEvt = 0;
						event.setNewAttending(Attending.SAVED);
						//NOTE: THIS CAN BE TESTED WITH PODUCTION BUILD ONLY
						FbUtil.handlePublishEvent(publishEventFragment, publishEventFragment, AppConstants.PERMISSIONS_FB_PUBLISH_EVT_OR_ART, 
	...
	private void onImgShareClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				 ...
				 */
				shareViaDialogFragment.show(((BaseActivityTab)FragmentUtil.getActivity(publishEventFragment))
	...
	private void onEventClick(final ViewHolder holder, final Event event) {
		...
		handler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				((EventListenerTab) FragmentUtil.getActivity(publishEventFragment)).onEventSelected(event, 
	...
	private void onHandleClick(final ViewHolder holder, final int position) {
		...
		if (holder.isSliderClose()) {
			...
			Animation slide = AnimationUtils.loadAnimation(FragmentUtil.getActivity(
					publishEventFragment), R.anim.slide_in_from_left);
			...
			Animation slide = AnimationUtils.loadAnimation(FragmentUtil.getActivity(
					publishEventFragment), android.R.anim.slide_out_right);
			slide.setAnimationListener(new AnimationListener() {
	...
	public void call(Session session, SessionState state, Exception exception) {
		...
		if (fbCallCountForSameEvt < AppConstants.MAX_FB_CALL_COUNT_FOR_SAME_EVT_OR_ART) {
			FbUtil.call(session, state, exception, publishEventFragment, publishEventFragment, AppConstants.PERMISSIONS_FB_PUBLISH_EVT_OR_ART, 
					AppConstants.REQ_CODE_FB_PUBLISH_EVT_OR_ART, eventPendingPublish);
			
		} else {
			fbCallCountForSameEvt = 0;
			publishEventFragment.setPendingAnnounce(false);
		}
	}

	public void onPublishPermissionGranted() {
		//Log.d(TAG, "onPublishPermissionGranted()");
		updateImgSaveSrc(holderPendingPublish, eventPendingPublish, FragmentUtil.getResources(publishEventFragment));
	}
